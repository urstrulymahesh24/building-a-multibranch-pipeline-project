pipeline {
   ...
   stage ('Start') {
      steps {
        ...
        slackSend (color: '#FFFF00', message: "STARTED: Succeded for ${env.GIT_REPO} by Git URL ${env.GIT_URL} Job name '${env.JOB_NAME} build no [${env.BUILD_NUMBER}]' Jenkins URL (${env.BUILD_URL})")
